...
microk8s kubectl port-forward --address localhost,{{LOCKSS_IPADDR}} service/lockss-configuration-service \
{{CFG_REST_PORT}}:{{CFG_DEFAULT_REST_PORT}}  {{CFG_UI_PORT}}:{{CFG_DEFAULT_UI_PORT}} {{LOCKSS_UI_PORT}}:{{CFG_DEFAULT_UI_PORT}}

microk8s kubectl port-forward --address localhost,{{LOCKSS_IPADDR}} service/lockss-metadata-extraction-service \
